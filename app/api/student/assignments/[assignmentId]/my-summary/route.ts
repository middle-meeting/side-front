...
export async function GET(request: NextRequest, { params }: { params: { assignmentId: string } }) {
  ...
  try {
    const backendUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/student/assignments/${assignmentId}/my-summary`;
    ...
    console.error(`Failed to fetch my-summary for assignment ${assignmentId}:`, error);
    return NextResponse.json(
      {
        status: "error",
        code: 500,
        message: "평가 요약을 불러오는 중 오류가 발생했습니다.",
        data: null,
        error: {
